...
0.6.x changes the Home Assistant domain from `grok_oauth` to `supergrok`. Old config entries will not migrate. Use 0.6.2 or later in HACS (0.6.0 had no release zip; 0.6.1 crashed setup without `voluptuous-openapi`).

1. Update **SuperGrok OAuth** to 0.6.2 in HACS (same custom repository: `helv-io/ha-supergrok`).
