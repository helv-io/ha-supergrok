...
0.6.x changes the Home Assistant domain from `grok_oauth` to `supergrok`. Old config entries will not migrate. Use 0.6.1 or later in HACS (0.6.0 had no release zip, so HACS reported "no content to download").

1. Update **SuperGrok OAuth** to 0.6.1 in HACS (same custom repository: `helv-io/ha-supergrok`).
